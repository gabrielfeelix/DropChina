...
function extrude(shape, depth, mat, name, bevel) {
  ...
  const geo = new THREE.ExtrudeGeometry(shape, {
    ...
    bevelThickness: b, bevelSize: b, bevelSegments: 3, curveSegments: 16
  });
...
function cyl(r, h, mat, name, seg = 40) {
...
function fanUnit(r, name, bladeCount = 11) {
  ...
  const ring = new THREE.Mesh(new THREE.TorusGeometry(r * 1.02, 0.005, 12, 48), MATS.graphite);
...
const BUILD = {
  'impressao-3d': () => {
    ...
    const nozzle = new THREE.Mesh(new THREE.CylinderGeometry(0.006, 0.020, 0.040, 28), MATS.ink);
    nozzle.name = 'nozzle';
  ...
  'audio-headsets': () => {
    ...
    const band = new THREE.Mesh(new THREE.TorusGeometry(0.188, 0.026, 24, 72, Math.PI), MATS.graphite);
    band.name = 'headband';
    g.add(at(band, 0, 0.210, 0));
    const bandPad = new THREE.Mesh(new THREE.TorusGeometry(0.158, 0.024, 20, 56, Math.PI * 0.66), MATS.ink);
    ...
    [-1, 1].forEach(sd => {
      ...
      const ring = new THREE.Mesh(new THREE.TorusGeometry(0.052, 0.008, 14, 44), MATS.teal);
      ring.name = 'accent_ring_' + id;
      g.add(at(ring, sd * 0.236, 0.112, 0, 0, Math.PI / 2, 0));
      const cushion = new THREE.Mesh(new THREE.TorusGeometry(0.070, 0.026, 18, 48), MATS.ink);
    ...
    boom.name = 'mic_boom';
    g.add(boom);
    const foam = new THREE.Mesh(new THREE.SphereGeometry(0.030, 32, 24), MATS.graphite);
    foam.name = 'mic_foam';
  ...
  perifericos: () => {
    ...
    const geo = new THREE.ExtrudeGeometry(outline, {
      ...
      bevelSize: 0.020, bevelSegments: 14, curveSegments: 40
    });
  ...
  'redes-internet': () => {
    ...
    [[0.10, 0.024, 24], [0.17, 0.020, 24]].forEach(([r, t], i) => {
      const arc = new THREE.Mesh(new THREE.TorusGeometry(r, t / 2, 16, 48, Math.PI * 0.62), MATS.teal);
  ...
  'cabos-adaptadores': () => {
    ...
    cord.name = 'cord';
    g.add(cord);
    const cap = new THREE.Mesh(new THREE.SphereGeometry(0.014, 20, 14), MATS.ink);
    cap.name = 'cord_end';
  ...
  'ofertas-da-semana': () => {
    ...
    [[-0.062, 0.058], [0.062, -0.058]].forEach(([x, y], i) => {
      const ring = new THREE.Mesh(new THREE.TorusGeometry(0.036, 0.011, 16, 44), MATS.graphite);
