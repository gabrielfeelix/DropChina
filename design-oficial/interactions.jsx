...
const megaCats = [
  {
    ...
    productIds: ["hp-667-duo", "hp-toner-premium", "evolut-toner"],
  },
  {
    name: "Toners",
    icon: "printer",
    items: ["Compatível Samsung", "HP CF283A", "Brother TN-1060", "Pantum PD-219", "Evolut Premium", "Kit revendedor"],
    productIds: ["evolut-toner", "byqualy-toner", "hp-toner-premium"],
  },
  {
    name: "Placas de Vídeo",
    icon: "gpu",
    items: ["NVIDIA GT 730", "GeForce GTX 1650", "RX 550 4GB", "GT 1030", "Para escritório", "Para games"],
    productIds: ["pcyes-rx550", "pcyes-mini-pc", "epson-wf-c5890"],
  },
  {
    name: "Computadores & Notebooks",
    icon: "storage",
    items: ["Mini PCs", "Notebooks", "All-in-One", "PCs gamer", "Workstations", "Acessórios"],
    productIds: ["pcyes-mini-pc", "epson-wf-c5890", "pcyes-rx550"],
  },
  {
    name: "Periféricos & Cabos",
    icon: "cable",
    items: ["Adaptadores HDMI / VGA", "Leitores USB", "Cabos USB-C", "Hubs USB", "Webcams Full HD", "Microfones"],
    productIds: ["hdmi-vga", "ugreen-reader", "hair-styler"],
  },
];

// Inline icon map (avoid cross-script scope issues with CategoryIcon)
const MegaCatIcon = ({ name }) => {
  const m = {
...
const MegaMenu = ({ open, initialIdx = 0, onClose, onMouseEnter }) => {
  ...
  const shown = (cat.productIds || []).map((id) => products.find((p) => p.id === id)).filter(Boolean);
  const list = shown.length ? shown : products.slice(0, 3);
  return (
    <div
      ...
      <div className="megamenu-inner wrap">
        <div className="megamenu-list">
          {megaCats.map((m, i) => (
            ...
            >
              <span className="megamenu-icon"><MegaCatIcon name={m.icon} /></span>
        ...
        <div className="megamenu-panel">
          <div className="megamenu-panel-head">
            ...
            <a href="Listagem (PLP).html">Ver tudo →</a>
          </div>
          <div className="megamenu-cols">
            <ul className="megamenu-sub">
              {cat.items.map((s) => (
                <li key={s}><a href="Listagem (PLP).html">{s}</a></li>
              ))}
            </ul>
              ...
              {list.map((p) => (
                <a key={p.id} href="#" className="megamenu-product">
                  <div className="megamenu-product-art">
                    {p.img ? (
                      <img src={p.img} alt={p.title} className="megamenu-product-img" />
                    ) : (
